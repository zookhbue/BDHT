classdef circularGraph1 < handle
% circularGraph1 Plot an interactive circular graph to illustrate connections in a network.
%
%% Syntax
% circularGraph1(X)
% circularGraph1(X,'PropertyName',propertyvalue,...)
% h = circularGraph1(...)
%
%% Description
...
% Copyright 2016 The MathWorks, Inc.
  ...
  methods
    function this = circularGraph1(adjacencyMatrix,varargin)
      % Constructor
      p = inputParser;
      ...
      this.ShowButton = uicontrol(...
        ...
        'Position',[0 40 80 40],...
        'String','Show All',...
        'Callback',@circularGraph1.showNodes,...
        'UserData',this);
      ...
      this.HideButton = uicontrol(...
        ...
        'Position',[0 0 80 40],...
        'String','Hide All',...
        'Callback',@circularGraph1.hideNodes,...
        'UserData',this);
      
      fig = gcf;
      set(fig,...
        'UserData',this,...
        'CloseRequestFcn',@circularGraph1.CloseRequestFcn);
...
%         this.Node(i).Color = this.ColorMap(i,:);
        this.Node(i).Color = [0.41 0.41 0.41];
        ...
        if row(i) ~= col(i)
          if abs(row(i) - col(i)) - length(adjacencyMatrix)/2 == 0 
            ...
            this.Node(row(i)).Connection(end+1) = line(...
              ...
              'PickableParts','none');
            point1 = [r*cos(theta(end-1))+x0, r*sin(theta(end-1))+y0];
            point2 = [r*cos(theta(end))+x0, r*sin(theta(end))+y0];
            d0 = atan2(point2(2)-point1(2),point2(1)-point1(1));
            s0 = sqrt( (point2(2)-point1(2))^2 + (point2(1)-point1(1))^2 )*2;
            point3 = point2 - [s0*cos(d0-pi/6), s0*sin(d0-pi/6)];
            point4 = point2 - [s0*cos(d0+pi/6), s0*sin(d0+pi/6)];
            line( [point3(1) point2(1)], [point3(2) point2(2)], 'LineWidth',lineWidth(i),'color',this.ColorMap(row(i),:));
            line( [point4(1) point2(1)], [point4(2) point2(2)], 'LineWidth',lineWidth(i), 'color',this.ColorMap(row(i),:));
%             annotation('arrow',[u(1) u(2)],[v(1) v(2)])
            ...
            this.Node(row(i)).Connection(end+1) = line(...
              ...
              'PickableParts','none');
%             this.Node(row(i)).Connection(end+1) = line(...
%               r*cos(theta(end-1:end))+x0,...
%               r*sin(theta(end-1:end))+y0,...
...
%               'PickableParts','none');
            ...
            line( [point3(1) point2(1)], [point3(2) point2(2)], 'LineWidth',lineWidth(i),'color',this.ColorMap(row(i),:));
            line( [point4(1) point2(1)], [point4(2) point2(2)], 'LineWidth',lineWidth(i), 'color',this.ColorMap(row(i),:));
%             line( [point1(1) point2(1)], [point1(2) point2(2)], 'LineWidth',4 );
%             this.Node(row(i)).Connection(end+1) = line([u(end-2) u(end)],[v(end-2) v(end)])
          end
        end
      end
